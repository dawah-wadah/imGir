...
def create_replies(comment, parent)
  comment[:children].each do |child|
        Comment.create({
          ...
          if !child[:children].empty? {
            child[:children].each do |reply|
            create_replies(reply, child)
            end
          }
...
post = Post.create({
    ...
    post[:comments].each do |foo|
      comment = Comment.create({
        ...
        if !foo[:children].empty?
        foo[:children].each do |child|
          create_replies(child, foo)
        end
      end
    end
end
